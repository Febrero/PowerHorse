...
contract HorsePortfolioAgent is Ownable, ReentrancyGuard {
    ...
    function executeInvestment(
        ...
        // Calculate shares to buy - iteratively find affordable amount
        uint256 estimatedShares = usdtAmount * 1e12; // Initial rough estimate
        ...
        // Keep halving until we find an affordable amount (max 20 iterations)
        uint256 iterations = 0;
        while (totalCost > usdtAmount && iterations < 20 && estimatedShares > 1e15) { // Allow down to 0.001 shares
        ...
        // Final check - ensure we can afford it
        if (totalCost > usdtAmount) revert InsufficientFunds();
        
        // Execute purchase
        usdt.forceApprove(address(bondingCurve), totalCost);
